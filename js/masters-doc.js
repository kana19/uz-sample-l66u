 ...
 *  - 商品マスタ(products)＝FAX受注OCRの名寄せ先（受け皿）。fax_order_ocr ON の店だけ設定画面に出す。
 *    書類発行（見積書・請求書）は商品マスタを使わない（2026-09-25 金光決定・請求書は売上と連携）。
 *  - 顧客マスタ(customers)＝見積/請求の宛名と FAX受注の発注元照合の台帳。doc_automation ON の店だけ設定画面に出す。
 *  - セクションの表示/非表示は app.js uzApplyFeatureGates（[data-feature="fax_order_ocr" / "doc_automation"]）が担当。
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (typeof uzDocAutomationEnabled === 'function') {
  ...
  // 商品マスタ（FAX受注の名寄せ先）は fax_order_ocr ON の店のみ（書類発行の ON/OFF とは独立）。
  _mdocFaxEnabled().then(function (on) {
    if (!on) return;
    _mdocBindProductAdd();
    mdocLoadProducts();
  });
...
/** FAX受注（fax_order_ocr）が有効か。判定不能時は false（安全側＝商品マスタ節は非表示のまま）。 */
function _mdocFaxEnabled() {
  if (typeof callGAS !== 'function') return Promise.resolve(false);
  return callGAS('getFaxOrderConfig').then(function (res) { return !!(res && res.enabled); }).catch(function () { return false; });
}
